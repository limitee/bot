...
{
	//AnsiString utf8Str =AnsiStringToUtf8(Input);
...
AnsiString __fastcall AnsiStringToUtf8(AnsiString Input){
	WCHAR str_w[16384] = {0};
	char str_utf8[16384] = {0};
	//mb->wc
	int len =MultiByteToWideChar(CP_ACP, 0, Input.c_str(), -1, NULL, 0);
	MultiByteToWideChar(CP_ACP,0,Input.c_str(),-1,str_w,len);
	//wc->utf8
	DWORD dwNum = WideCharToMultiByte(CP_UTF8, NULL, str_w, -1, NULL, 0, NULL, FALSE);
	WideCharToMultiByte (CP_UTF8, NULL, str_w, -1, str_utf8, dwNum, NULL, FALSE);

	return AnsiString(str_utf8);
}
