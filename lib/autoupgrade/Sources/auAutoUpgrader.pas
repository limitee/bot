...
{$ENDIF}
  ...
  PrimaryLanguages: Array[TauAutoUpgraderLanguage] of Integer = (0,
                      ...
                      LANG_ESTONIAN,   LANG_UKRAINIAN,  LANG_CZECH,
                      LANG_HUNGARIAN,  LANG_ROMANIAN,   LANG_CATALAN,
                      LANG_BASQUE,     LANG_AFRIKAANS,  LANG_CHINESE,
                      LANG_CHINESE,    LANG_INDONESIAN, LANG_MALAY,
                      LANG_ICELANDIC);
var
...
{ acHTTP events }
procedure TauAutoUpgrader.HTTPHeaderInfo(Sender: TObject; const RawHeadersCRLF, ContentType, ContentLanguage, ContentEncoding, Location: String;
                                         ContentLength: Integer; Date, LastModified, Expires: TDateTime; var ContinueDownload: Boolean);
var
...
begin
  ...
  if Assigned(WizardForm) then
    WizardForm.FileSizeLab.Caption := AUFmtLangStr(auFileSize, [ContentLength / 1024]);
  ...
  if Assigned(WizardForm) then
   with WizardForm do
    begin
     ...
     EstimatedTimeLeftLab.Caption := AUFmtLangStr(auEstimatedTime, [EstimatedTimeLeft div 60 div 60,
  ...
  if FIsAutoCheck then
   begin
    ...
    if (mHostUnreachable in FShowMessages) then
      AUAdvErrorBox(AUFmtLangStr(auHostUnreachable, ['' + HTTPPrefix + HostName + '''']));
...
begin
  ...
  if IsUpdateAvailable and (FDownloadQueue.Count <> 0) then
   begin
    ...
    if CanUpgrade and (not (mAskUpgrade in FShowMessages) or ShowUpgradeBox(MsgText, False)) then
...
begin
  ...
  WinExec(PChar(UpgraderFileName + ' ' +
                Application.ExeName + ' ' + ParamStr),
          SW_SHOWNORMAL);
